 ...
 *Author: Mist
 *Date: 2024-06-12
 *vx臭宝乐园 Authorization 
 * cron "10 9 * * *" cbly.js
 * export cbly= Authorization 多账号换行或者#分隔
 */
// ============================================================================================================
const $ = new Env('vx臭宝乐园') 
const axios = require('axios')
const env_name = 'cbly' //环境变量名字
...
!(async () => {
    ...
    await getVersion("yang7758258/ohhh154@main/cbly.js");
    await main();//主函数
...
async function SignTask(user) {
    try {
        let urlObject = {
            ...
            data: {
                "appletVersion": "2.0.31",
                "phoneSystem": "Windows 11 x64",
                "phoneModel": "microsoft",
                "functionName": "签到",
                "module": "首页",
                "linkUrl": "pages/signIn/signIn",
                "secondPage": "每日签到"
            }
        }
        //
        ...
        if (result?.status == '200') {
            //打印签到结果
            DoubleLog(`🌸账号[${user.index}]` + `🕊签到状态[${result.msg}]🎉`);
...
async function account(user) {
    try {
        ...
        //console.log(result);
        if (result?.status == '200') {
            DoubleLog(`🌸账号[${user.index}]🕊账户当前${result.data.points}积分💰，签到${result.data.totalSignNum}天🎉`);
        }else {
