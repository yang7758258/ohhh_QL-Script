 ...
 * Author: Mist
 * Date: 2024-07-03
 * cron "30 8,18 * * *" astc.js
 * export Astc= openid 多账号换行或者#分隔
 */
// ============================================================================================================
const $ = new Env('vx阿叔同城') 
const axios = require('axios')
const env_name = 'Astc' //环境变量名字
...
!(async () => {
    ...
    await getVersion("yang7758258/ohhh154@main/astc.js");
    await main();//主函数
...
async  userTask(user) {
    ...
    await this.readid(user)
    await wait(1);
    await this.readid(user)
    await wait(1);
    await this.goods(user)
    await wait(1);
...
//签到
    async  SignTask(user) {
        try {
            ...
            let urlObject = {
                method: 'post',
                url: `https://ztvip1.cn/api/o2o.Integral/Signindex?appid=1491&appkey=4b6577fb535885831ca6aa9ecbcb72ed&version=v3.9.1.8&app_version=v3.9.3.12&openid=${user.openid}&invite&pageid=qiandao&time=1720013706&source=wx_mp&lang=zh_CN`,
            ...
            if (result?.code == 200) {
                //打印签到结果
                DoubleLog(`🕊账号[${user.index}] 签到成功,签到[${result.data.user.day_num}]天🎉`);
            }else{
...
async  liulan(user) {
    try {
        //DoubleLog(`🕊账号[${user.index}] 其余任务日志省略...`);
        console.log(`🕊账号[${user.index}] 开始浏览任务...`);
        let urlObject = {
            method: 'post',
            url: `https://ztvip1.cn/api/o2o.integralrenwu/fenlei?appid=1491&appkey=4b6577fb535885831ca6aa9ecbcb72ed&version=v3.9.1.8&app_version=v3.9.3.12&openid=${user.openid}&invite&pageid=tongcheng&time=1720014919&source=wx_mp&lang=zh_CN`,
            headers: this.headers,
            data : {
                "id": "373109",
                "lat": 27.35264015197754,
                "lng": 109.17491912841797
              }
        }
        //console.log(urlObject);
        let { data: result} = await axios.request(urlObject)
        if (result?.code == 200) {
            //打印签到结果
            console.log(`🕊账号[${user.index}] 浏览成功🎉`);
        }else {
            console.log(`🕊账号[${user.index}] 浏览失败:${result.message}🚫`)
...
async  readid(user) {
    try {
        //console.log(`🕊账号[${user.index}] 开始阅读任务...`);
        let urlObject = {
            method: 'post',
            url: `https://ztvip1.cn/api/o2o.qianggou/index?appid=1491&appkey=4b6577fb535885831ca6aa9ecbcb72ed&version=v3.9.1.8&app_version=v3.9.3.12&openid=${user.openid}&invite&pageid=qianggou&time=1720015571&source=wx_mp&lang=zh_CN`,
            headers: this.headers,
            data : {
                "cat_id": 0,
                "page": 1,
        ...
        if (result?.code == 200) {
            //打印签到结果
            console.log(`🕊账号[${user.index}] 获取抢购id成功🎉`);
            this.id = result.data.list[0].qinggou_id;
            //console.log(this.id);
            await this.read(user);
        }else {
            console.log(`🕊账号[${user.index}] 获取抢购id失败:${result.message}🚫`)
        }
        } catch (e) {
        console.log(e);
        }
...
async  read(user) {
    try {
        DoubleLog(`🕊账号[${user.index}] 开始浏览抢购任务...`);
        let urlObject = {
            method: 'post',
            url: `https://ztvip1.cn/api/o2o.integralrenwu/qianggou?appid=1491&appkey=4b6577fb535885831ca6aa9ecbcb72ed&version=v3.9.1.8&app_version=v3.9.3.12&openid=${user.openid}&invite&pageid=qianggou&time=1720015571&source=wx_mp&lang=zh_CN`,
            headers: this.headers,
            data : {
                "qinggou_id": `${this.id}`
              }
       ...
       // console.log(result);
        if (result?.code == 200) {
            //打印签到结果
            DoubleLog(`🕊账号[${user.index}] 浏览抢购成功🎉`);
        }else {
            DoubleLog(`🕊账号[${user.index}] 浏览抢购任务失败:${result.message}🚫`)
        }
        } catch (e) {
        console.log(e);
        }
...
async  goods(user) {
    try {
       ...
       // console.log(result);
        if (result?.code == 200) {
            //打印签到结果
            DoubleLog(`🕊账号[${user.index}] 浏览商品成功🎉`);
        }else {
            DoubleLog(`🕊账号[${user.index}] 浏览商品任务失败:${result.message}🚫`)
        }
        } catch (e) {
        console.log(e);
        }
...
async  account(user) {
    try {
        ...
        let urlObject = {
            method: 'post',
            url: `https://ztvip1.cn/api/o2o.integral/chongzhi?appid=1491&appkey=4b6577fb535885831ca6aa9ecbcb72ed&version=v3.9.1.8&app_version=v3.9.3.12&openid=${user.openid}&invite&pageid=qianggou&time=1720015571&source=wx_mp&lang=zh_CN`,
            headers: this.headers,
            data : {
       ...
       // console.log(result);
        if (result?.code == 200) {
            //打印签到结果
            DoubleLog(`🕊账号[${user.index}] 查询成功，当前[${result.data.user.integral}]金币🎉`);
