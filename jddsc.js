...
!(async () => {
    ...
    await getVersion("yang7758258/ohhh154@main/jddsc.js");
    await main();//主函数
...
async function SignTask(user) {
    ...
    try {
        let urlObject = {
            method: 'get',
            url: 'https://www.jindd.shop/addons/yun_shop/api.php?i=12&uuid=0&type=1&mid=43808&version=v1.1.137&validate_page=1&route=plugin.sign.Frontend.Modules.Sign.Controllers.sign.sign',
            headers: {
                'Host': 'www.jindd.shop',
                'cookie': user.cookie,
...
// 做任务
async function missiontask(user,id) {
    try {
        let urlObject = {
            method: 'post',
            fn: 'missiontask',
            url: "https://www.jindd.shop/addons/yun_shop/api.php?i=12&uuid=0&type=1&mid=43808&version=v1.1.137&validate_page=1&route=plugin.qmtask.api.qmtask.confirm_qmtask",
            headers: {
                'Host': 'www.jindd.shop',
                'cookie': user.cookie,
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.0.0 Safari/537.36 MicroMessenger/7.0.20.1781(0x6700143B) NetType/WIFI MiniProgramEnv/Windows WindowsWechat/WMPF WindowsWechat(0x63090a13) XWEB/9129',
        ...
        //console.log(result);
        if (result?.result == "1") {
            DoubleLog(`🌸账号[${user.index}]🕊任务[${id}]成功🎉`)
...
//提现
async function tixian(user) {
    try {
        let urlObject = {
            method: 'get',
            fn: 'tixian',
            url: "https://www.jindd.shop/addons/yun_shop/api.php?i=12&uuid=0&type=1&mid=43808&version=v1.1.137&validate_page=1&route=plugin.love.Frontend.Modules.Love.Controllers.withdraw.index&change_value=1.5&withdraw_type=4",
            headers: {
                'Host': 'www.jindd.shop',
                'cookie': user.cookie,
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.0.0 Safari/537.36 MicroMessenger/7.0.20.1781(0x6700143B) NetType/WIFI MiniProgramEnv/Windows WindowsWechat/WMPF WindowsWechat(0x63090a13) XWEB/9129',
            ...
            //body: `{"isReward":false}`   请求体，get方法没有请求体  httpRequest
            //form: {"id":id,"mid":"43808","mark":null,"mark_id":null,"enter_at":"","leave_at":""}
        };
        const { statusCode, headers, result } = await request(urlObject)
        //console.log(result);
        if (result?.result == "1") {
            DoubleLog(`🌸账号[${user.index}]🕊默认提现1.5[${result.msg}]🎉`)
        }else DoubleLog(`🌸账号[${user.index}]🕊提现失败,不足1.5❌`)
    } catch (e) {
        //打印错误信息
        console.log(e)
    }
...
//查询接口
async function chaxun(user) {
    try {
        let urlObject = {
            method: 'get',
            fn: 'chaxun',
            url: "https://www.jindd.shop/addons/yun_shop/api.php?i=12&uuid=0&type=1&mid=43808&version=v1.1.137&validate_page=1&route=finance.balance.balance&i=12&type=1",
            headers: {
                'Host': 'www.jindd.shop',
                'cookie': user.cookie,
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.0.0 Safari/537.36 MicroMessenger/7.0.20.1781(0x6700143B) NetType/WIFI MiniProgramEnv/Windows WindowsWechat/WMPF WindowsWechat(0x63090a13) XWEB/9129',
        ...
        //console.log(statusCode, headers, result);
        if (result?.result == "1") {
            DoubleLog(`🌸账号[${user.index}]当前钱包余额💰:[${result.data.credit2}]🎉`)
        } else{
            //打印请求错误信息
            DoubleLog(`🌸账号[${user.index}]余额查询失败`)
        }
    } catch (e) {
        //打印错误信息
        console.log(e)
