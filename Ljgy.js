...
!(async () => {
    ...
    await getVersion("yang7758258/ohhh154@main/Ljgy.js");
    await main();//主函数
...
async function userTask(user) {
    console.log(`\n============= 账号[${user.index}]开始任务 =============`)
    await SignTask(user,'https://wep.qzlcis.com/api/user/sign')
    await wait (1)
    await SignTask(user,'http://wep.qzlcis.com/api/user/clock')
}
// =============================================================================================================================
//签到、打卡
async function SignTask(user,url) {
    try {
        ...
        if (result?.code == '1') {
            //打印签到结果
            DoubleLog(`🌸账号[${user.index}]` + `🕊签到成功,获得现金[${result.data.reward_num}]元🎉`);
        }if(result?.code == "0") {
            DoubleLog(`🌸账号[${user.index}]失败:${result.info}❌`)
        }if (result?.code == "1") {
            DoubleLog(`🌸账号[${user.index}]今日打卡:${result.info}🎉`)
        }
