 ...
 * Author: Mist
 * Date: 2024-07-03
 * cron "30 8,18 * * *" mpcbh.js
 * export mpcbh= authorization 多账号换行或者#分隔
 */
// ============================================================================================================
const $ = new Env('vx毛铺草本荟') 
const axios = require('axios')
const md5 = require('md5')
const env_name = 'mpcbh' //环境变量名字
...
!(async () => {
    ...
    await getVersion("yang7758258/ohhh154@main/mpcbh.js");
    await main();//主函数
        ...
        try {
            ...
            let urlObject = {
                ...
                headers: {
                    ...
                    "referer": "https://servicewechat.com/wxefd0fe341e06b815/318/page-frame.html",
            ...
            if (result?.code == 0) {
                //打印签到结果
                DoubleLog(`🕊账号[${user.index}] 签到成功,获得积分[${result.data.point_today}]🎉`);
            }else{
...
async  readid(user) {
    try {
        ...
        DoubleLog(`🕊账号[${user.index}] 开始获取阅读任务id...`);
        let urlObject = {
            method: 'get',
            url: `https://mpb.jingjiu.com/proxy-he/api/FlanArticle/articleList?column_id=renhe-hewenhua&pageNo=1&pageSize=10`,
            headers: {
                "x-version": "0.0.1",
                "xweb_xhr": "1",
                "authorization": user.authorization,
        ...
        if (result?.code == 0) {
            //打印签到结果
            DoubleLog(`🕊账号[${user.index}] 阅读任务id获取成功🎉`);
            let read_id = []
            for (let i = 0; i < result.data.articleList.rows.length; i++) {
                read_id.push(result.data.articleList.rows[i].article_id);
            }
            this.id = read_id;
        }else {
            DoubleLog(`🕊账号[${user.index}] 阅读任务id获取失败:${result.message}🚫`)
...
async  reads(user,id) {
    try {
        await this.get_sign1(user,id);
        //console.log(`🕊账号[${user.index}] 开始阅读任务...`);
        let urlObject = {
            method: 'post',
            url: `https://mpb.jingjiu.com/proxy-he/api/FlanArticle/articleView`,
            headers: {
                "x-version": "0.0.1",
                "xweb_xhr": "1",
                "apptime": this.time,
        ...
        if (result?.code == 0) {
            //打印签到结果
            console.log(`🕊账号[${user.index}] 阅读成功,获得积分[${result.data.task.point}]🎉`);
            //this.ticket = result.result.ticket;
        }else {
            console.log(`🕊账号[${user.index}] 阅读任务失败:${result.message}🚫`)
        }
        } catch (e) {
        console.log(e);
        }
...
async  read(user,id) {
    try {
        ...
        let urlObject = {
            method: 'get',
            url: `https://mpb.jingjiu.com/proxy-he/api/FlanArticle/articleDetail?article_id=${id}`,
            headers: {
                "x-version": "0.0.1",
                "xweb_xhr": "1",
                "authorization": user.authorization,
       ...
       // console.log(result);
        if (result?.code == 0) {
            //打印签到结果
            console.log(`🕊账号[${user.index}] 阅读成功🎉`);
            await this.reads(user,id);
        }else {
            console.log(`🕊账号[${user.index}] 阅读任务失败:${result.message}🚫`)
        }
        } catch (e) {
...
async  account(user) {
    try {
        DoubleLog(`🕊账号[${user.index}] 查询账户积分...`);
        let urlObject = {
            method: 'get',
            url: `https://mpb.jingjiu.com/proxy-he/api/FlanUser/logPointList?type&month=7&pageNo=1&pageSize=10`,
            headers: {
                "x-version": "0.0.1",
                "xweb_xhr": "1",
                "authorization": user.authorization,
        ...
        let { data: result} = await axios.request(urlObject)
         if (result?.code == 0) {
                //打印签到结果
                DoubleLog(`🕊账号[${user.index}] 查询成功,总积分[${result.data.my_point}]🎉`);
                //this.cishu = result.result.lottery_count
        }else {
...
async get_sign(user) {
    ...
    this.sign = `${this.time}` + `${s}` + "DYSHJS^M&.YXZRGS" + `${user.authorization}`
    this.appign = md5(this.sign).toUpperCase().substr(-10);
}
async get_sign1(user,id) {
    let currentDate = new Date();
    this.formattedDate = currentDate.toISOString().split('T')[0];
    this.time = Math.floor(new Date().getTime() / 1000);
    this.data = {
