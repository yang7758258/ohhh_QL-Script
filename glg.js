 ...
 * Date: 2024-06-25
 * cron "30 7 * * *" zwxq.js 
 * export glg= 'X-Auth-Token'     多账号换行或者#分隔
 */
// ============================================================================================================
const $ = new Env('vx格力高') 
const axios = require('axios')
//const md5 = require('md5')
const env_name = 'glg' //环境变量名字
...
!(async () => {
    ...
    await getVersion("yang7758258/ohhh154@main/glg.js");
    await main();//主函数
...
async function Sign(user) {
    try {
        let urlObject = {
            method: 'put',
            url: `https://crm.glico.cn/miniapp/member/checkin`,
            headers: {
                'Host': 'crm.glico.cn',
...
async function Draw(user) {
    try {
        let urlObject = {
            method: 'post',
            url: `https://crm.glico.cn/miniapp/member/lotteries/submit/7309645?addressId`,
            headers: {
                'Host': 'crm.glico.cn',
                'X-Auth-Token': user.Token,
                'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.0.0 Safari/537.36 MicroMessenger/7.0.20.1781(0x6700143B) NetType/WIFI MiniProgramEnv/Windows WindowsWechat/WMPF WindowsWechat(0x63090b11) XWEB/9129',
