 ...
 *Author: Mist
 *Date: 2024-06-09
 * 抓 vx小程序健达福利社 KUMI-TOKEN&PROJECT-ID 积分换实物
 * cron "10 11,12 * * *" jdfls.js
 * export jdfls= KUMI-TOKEN&PROJECT-ID 多账号换行或者#分隔
 */
// ============================================================================================================
const $ = new Env('vx健达福利社') 
const axios = require('axios')
const env_name = 'jdfls' //环境变量名字
...
!(async () => {
    ...
    await getVersion("yang7758258/ohhh154@main/jdfls.js");
    await main();//主函数
...
async function SignTask(user) {
    try {
        let urlObject = {
            method: 'post',
            url: 'https://mole.ferrero.com.cn/boss/boss/kinder/interaction/signin/record/create',
            headers: {
                'Host': 'mole.ferrero.com.cn',
                'Content-Type': 'application/json',
                'PROJECT-ID': user.id,
                'PLATFORM': 'KUMI_KINDER',
                'KUMI-TOKEN': user.TOKEN,
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.0.0 Safari/537.36 MicroMessenger/7.0.20.1781(0x6700143B) NetType/WIFI MiniProgramEnv/Windows WindowsWechat/WMPF WindowsWechat(0x63090a13) XWEB/9129',
            },
            data: {
                "activityId":"230309eSgKaUHr2t",
            }
        }
        //
        ...
        if (result?.code == "1") {
            //打印签到结果
            DoubleLog(`🌸账号[${user.index}]` + `🕊签到成功，获得[${result.data.dayPrize.prizeName}]🎉`);
        }if(result?.code == "400") {
...
// 抽奖
async function dw(user) {
    try {
        let urlObject = {
            method: 'post',
            url: 'https://mole.ferrero.com.cn/boss/boss/lottery/v1/record/create',
            headers: {
                'Host': 'mole.ferrero.com.cn',
                'Content-Type': 'application/json',
                'PROJECT-ID': user.id,
            ...
            data: {
                "lotteryId":"ffffffffcb57dfb0ffffffffee417c91",
            }
        };
        let { data: result } = await axios.request(urlObject)
        //console.log(result);
        if (result?.code == "1") {
            DoubleLog(`🌸账号[${user.index}]🕊抽奖获得[${result.data.prizeName}]🎉`)
        }else {
...
async function account(user) {
    try {
        let urlObject = {
            method: 'post',
            url: 'https://mole.ferrero.com.cn/boss/boss/scrm/member/point/get',
            headers: {
                'Host': 'mole.ferrero.com.cn',
        ...
        //console.log(result);
        if (result?.code == "1") {
            DoubleLog(`🌸账号[${user.index}]🕊账户当前积分${result.data.point}💰`)
        }else {
