 ...
 */
// ============================================================================================================
const $ = new Env('vx汤臣倍健汤星球') 
const axios = require('axios')
const env_name = 'tcbjtxq' //环境变量名字
...
!(async () => {
    ...
    await getVersion("yang7758258/ohhh154@main/tcbjtxq.js");
    await main();//主函数
...
async function SignTask(user) {
    try {
        ...
        if (result?.data.rspCode == '00') {
            DoubleLog(`🌸账号[${user.index}]` + `🕊签到成功,获得${result.data.result.dailyPointReward}积分🎉`);
        }if(result?.data.rspCode == "SIGN_TODAY_ALREADY_DONE") {
            DoubleLog(`🌸账号[${user.index}]签到失败:${result.data.rspMsg}❌`)
        }if (result?.code == "500") {
...
async function account(user) {
    try {
        let urlObject = {
            method: 'get',
            url: 'https://vip.by-health.com/vip-api/member/point/page?page=1&rows=15',
            headers: {
                'Host': 'vip.by-health.com',
                'Content-Type': 'application/json',
                'authorization': user.Authorization,
        ...
        //console.log(result);
        if (result?.data.rspCode == '00') {
            DoubleLog(`🌸账号[${user.index}]🕊今日获得[${result.data.result.results[0].point}]积分，总积分：[${result.data.result.results[0].totalPoint}]💰`)
        }else {
            DoubleLog(`🌸账号[${user.index}]🕊查询当前积分失败❌`)
        }
    } catch (e) {
        //打印错误信息
        console.log(e)
