...
!(async () => {
    await getNotice();
    await getVersion("yang7758258/ohhh154@main/emsyhzx.js");
    await main();
    await SendMsg(msg);
...
async  Login(user) {
    try {
        ...
        if (result?.code == '000000') {
            ...
            this.userId = result.info.memberId
        }else{
...
async  sign(user) {
    try {
        DoubleLog(`🕊账号[${user.index}] 去签到...`);
            
        //console.log(formattedDate);
        let urlObject = {
            method: 'post',
            url: `https://ump.ems.com.cn/activCenterApi/signActivInfo/sign`,
            headers: this.headers,
            data: {
                "userId": this.userId,
                "appId": "wx52872495fb375c4b",
...
async  Reward(user) {
    try {
        DoubleLog(`🕊账号[${user.index}] ⌛️...`);
            
        //console.log(formattedDate);
        let urlObject = {
            method: 'post',
            url: `https://ump.ems.com.cn/activCenterApi/signActivInfo/querySignDetail`,
            headers: this.headers,
            data: {
                "userId": this.userId,
                "appId": "wx52872495fb375c4b",
        ...
        if (result?.code == '000000') {
                //打印签到结果
            DoubleLog(`🕊账号[${user.index}] 当前累计签到[${result.info.signDay}]天🎉`);
        }else{
...
async  account(user) {
    try {
        ...
        let urlObject = {
            method: 'post',
            url: `https://ump.ems.com.cn/memberCenterApiV2/golds/memberGoldsInfo`,
            headers: {
                "Content-Type": 'application/json',
                'MC-TOKEN': this.token,
                "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 17_4_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.48(0x18003030) NetType/WIFI Language/zh_CN",
        ...
        if (result?.code == '000000') {
            //打印签到结果
            DoubleLog(`🕊账号[${user.index}] 查询成功:总积分[${result.info.availableGoldsTotal}]🎉`);
