...
async function ShareDailyScore(user) {
    try {
        ...
        if (result?.state == true) {
            ...
            DoubleLog(`🌸账号[${user.index}]分享-失败:${result.msg}❌`)
        }
    } catch (e) {
        //打印错误信息
...
async function UserSign(user) {
    try {
        ...
        if (result?.state == true) {
            ...
            DoubleLog(`🌸账号[${user.index}]水滴签到-失败:${result.msg}❌`)
        }
...
async function UserShare(user) {
    try {
        ...
        if (result?.state == true) {
            ...
            DoubleLog(`🌸账号[${user.index}]任务分享领水滴-失败:${result.msg}❌`)
        }
...
async function GetUserPoint(user) {
    try {
        ...
        if (result?.state == true) {
            ...
            DoubleLog(`🌸账号[${user.index}]积分查询失败:${result.msg}❌`)
        }
