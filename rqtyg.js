...
!(async () => {
    ...
    await getVersion("yang7758258/ohhh154@main/rqtyg.js");
    await main();//主函数
...
async function SignInDailyScore(user) {
    try {
        ...
        let urlObject = {
            method: 'get',
            url: `https://prod-api.nissinfoodium.com.cn/gw-shop/app/v1/signs/sign?date=${time}&type=1`,
            headers: {
        ...
        if (result?.code == '200') {
            //打印签到结果
            DoubleLog(`🌸账号[${user.index}]` + `🕊${result.data.message}🎉`);
        }else{
...
async function Draw(user) {
    try {
        let urlObject = {
            method: 'post',
            url: `https://prod-api.nissinfoodium.com.cn/gw-shop/app/v1/activity-center/luck-draw/initiate`,
            headers: {
                'Host': 'prod-api.nissinfoodium.com.cn',
                'content-type': 'application/json',
                'enterprise-hash': '1246b9ecd0972c7b0e50b4c9cdad9f0c',
        ...
        if (result?.code == '200') {
            //打印签到结果
            DoubleLog(`🌸账号[${user.index}]` + `🕊抽奖成功-获得${result.data.draw_prize_name}🎉`);
        }else {
...
async function GetUserPoint(user) {
    try {
        ...
        if (result?.code == '200') {
            //打印签到结果
            DoubleLog(`🌸账号[${user.index}]` + `🕊账户当前积分[${result.data.count.score}]💰`);
        }else{
