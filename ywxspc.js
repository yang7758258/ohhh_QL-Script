 ...
 * Date: 2024-07-05
 * cron "59 7 * * *" ywxspc.js
 * export ywxspc= x-auth-token 多账号换行或者#分隔
 */
// ============================================================================================================
const $ = new Env('义务小商品城') 
const axios = require('axios')
const env_name = 'ywxspc' //环境变量名字
...
!(async () => {
    await getNotice();
    await getVersion("yang7758258/ohhh154@main/ywxspc.js");
    await main();
    await SendMsg(msg);
...
class run {
    constructor(user) {
        ...
        this.id = ['1076315','1075605','1075751']
        this.headers = {
...
//
    async  sign(user) {
        try {
            ...
            let urlObject = {
                method: 'post',
                url: `https://apiserver.chinagoods.com/bpmms/sign/seller/signup`,
            ...
            if (result?.code == '20000') {
                //打印签到结果
                DoubleLog(`🕊账号[${user.index}] 签到:[${result.message}] 第[${result.data.totalDays}]天🎉`);
            }else{
...
async  account(user) {
    try {
        DoubleLog(`🕊账号[${user.index}] 开始查询任务...`);
        let urlObject = {
            method: 'get',
            url: `https://apiserver.chinagoods.com/growth/score/record/page?pageNo=1&pageSize=10&month=7&year=2024&userType=BUYER`,
            headers: this.headers,
        ...
        if (result?.code == '20000') {
            //打印签到结果
            DoubleLog(`🕊账号[${user.index}] 查询成功:总积分[${result.data.current}]🎉`);
