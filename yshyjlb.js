 ...
 *Author: Mist
 *Date: 2024-06-08
 * 抓 vx小程序仰韶会员俱乐部 Authorization 积分换实物
 * cron "10 10,16 * * *" yshyjlb.js
 * export yshyjlb= Authorization 多账号换行或者#分隔
 */
// ============================================================================================================
const $ = new Env('vx仰韶会员俱乐部') 
const axios = require('axios')
const env_name = 'yshyjlb' //环境变量名字
...
!(async () => {
    ...
    await getVersion("yang7758258/ohhh154@main/yshyjlb.js");
    await main();//主函数
...
async function SignTask(user) {
    try {
        let urlObject = {
            method: 'post',
            url: 'https://hy.51pt.top/app/ys/signIn/save',
            headers: {
                'Host': 'hy.51pt.top',
        ...
        if (result?.code == "200") {
            //打印签到结果
            DoubleLog(`🌸账号[${user.index}]` + `🕊[${result.data}]🎉`);
        }if(result?.code == "400") {
...
// 补签卡1
async function bq(user) {
    try {
        let urlObject = {
            method: 'get',
            url: 'https://hy.51pt.top/app/ys/appletTask/doTask?taskId=6',
            headers: {
                'Host': 'hy.51pt.top',
                'Authorization': user.Authorization,
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.0.0 Safari/537.36 MicroMessenger/7.0.20.1781(0x6700143B) NetType/WIFI MiniProgramEnv/Windows WindowsWechat/WMPF WindowsWechat(0x63090a13) XWEB/9129',
        ...
        //console.log(result);
        if (result?.code == "200") {
            DoubleLog(`🌸账号[${user.index}]🕊任务一完成[${result.msg}]🎉`)
        }else {
            DoubleLog(`🌸账号[${user.index}]🕊任务失败❌${result.msg}`)
        } 
    } catch (e) {
        //打印错误信息
        console.log(e)
...
async function jl(user) {
    try {
        let urlObject = {
            method: 'get',
            url: 'https://hy.51pt.top/app/ys/appletTask/doTask?taskId=7',
            headers: {
                'Host': 'hy.51pt.top',
                'Authorization': user.Authorization,
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.0.0 Safari/537.36 MicroMessenger/7.0.20.1781(0x6700143B) NetType/WIFI MiniProgramEnv/Windows WindowsWechat/WMPF WindowsWechat(0x63090a13) XWEB/9129',
...
async function account(user) {
    try {
        let urlObject = {
            method: 'get',
            url: 'https://hy.51pt.top/app/ys/convert/getMemberScore',
            headers: {
                'Host': 'hy.51pt.top',
                'Authorization': user.Authorization,
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.0.0 Safari/537.36 MicroMessenger/7.0.20.1781(0x6700143B) NetType/WIFI MiniProgramEnv/Windows WindowsWechat/WMPF WindowsWechat(0x63090a13) XWEB/9129',
        ...
        //console.log(result);
        if (result?.code == "200") {
            DoubleLog(`🌸账号[${user.index}]🕊账户当前积分${result.data}]💰`)
        }else {
