...
for account in accounts:
    ...
    for task in tasks:
        ...
        for _ in range(task_count):
            if task_type == 3 or task_type == 5 or task_type == 2:  # 分享、点赞和阅读任务
                # 生成随机的 targetId 和 targetType
                target_id = f"2{random.randint(100000, 999999)}"
                target_type = f"{random.randint(10, 99)}"

                # 签到签名
                sign_str = f"OMKCy2UxZwn8e4Ak{params_base['appName']}{params_base['app_id']}{params_base['os']}{params_base['osVer']}{target_id}{target_type}{current_timestamp}{token}{task_type}{uid}{params_base['vCode']}{params_base['vName']}"
            else:
                # 其他任务签名
            ...
            if task_type == 3 or task_type == 5 or task_type == 2:  
