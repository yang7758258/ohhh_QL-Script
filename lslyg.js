 ...
 * Date: 2024-08-31
 * cron "0 9 * * *" lslyg.js
 * export lslyg= x-account-id&x-access-token 多账号换行或者#分隔
 */
// ============================================================================================================
const $ = new Env('冷酸灵牙膏') 
const axios = require('axios')
const env_name = 'lslyg' //环境变量名字
...
!(async () => {
    await getNotice();
    await getVersion("yang7758258/ohhh154@main/lslyg.js");
    await main();
    await SendMsg(msg);
...
async  Login(user) {
    try {
        DoubleLog(`🕊账号[${user.index}] 签到...`);
            
        //console.log(formattedDate);
        let urlObject = {
            method: 'post',
            url: `https://consumer-api.quncrm.com/modules/campaigncenter/signin?maijsVersion=1.50.0&clientId=0191a818-750c-56ee-0ba8-8442d8ff94f0&appVersion=1.94.0.2368f2fbb1c&appName=群脉电商&envVersion=release&clientTime=2024-08-31T19:01:42.280+08:00`,
        ...
        let { data: result} = await axios.request(urlObject)
            //console.log(result);
        if (result?.rewardGroup) {
                //打印签到结果
            DoubleLog(`🕊账号[${user.index}] 签到成功，获得[${result.rewardGroup[1].score}]积分🎉`);
            
        }else{
            DoubleLog(`🕊账号[${user.index}] 签到失败:${e.response.data.message}⛔`)
        }
    } catch (e) {
        //this.result = e.data.message
        DoubleLog(`🕊账号[${user.index}] 签到失败:${e.response.data.message}⛔`);
    }
}
//点赞
async  sign(user) {
    try {
        DoubleLog(`🕊账号[${user.index}] 完成点赞任务...`);
            
        //console.log(formattedDate);
        let urlObject = {
            method: 'post',
            url: `https://consumer-api.quncrm.com/modules/forum/article/add-to-fav/66cfd221cb668d5373070f54?maijsVersion=1.50.0&clientId=0191a818-750c-56ee-0ba8-8442d8ff94f0&appVersion=1.94.0.2368f2fbb1c&appName=群脉电商&envVersion=release&clientTime=2024-08-31T19:18:04.856+08:00`,
            headers: this.headers,
            data: {
        ...
        if (result) {
                //打印签到结果
            DoubleLog(`🕊账号[${user.index}] 任务成功：获得[5]积分🎉`);
...
async  Reward(user) {
    try {
        console.log(`🕊账号[${user.index}] ⌛️...`);
            
        //console.log(formattedDate);
        let urlObject = {
            method: 'post',
            url: `https://consumer-api.quncrm.com/modules/forum/article/remove-from-fav/66cfd221cb668d5373070f54?maijsVersion=1.50.0&clientId=0191a818-750c-56ee-0ba8-8442d8ff94f0&appVersion=1.94.0.2368f2fbb1c&appName=群脉电商&envVersion=release&clientTime=2024-08-31T19:19:59.303+08:00`,
            headers: this.headers,
            data: {
        ...
        if (result) {
                //打印签到结果
            console.log(`🕊账号[${user.index}] 任务成功🎉`);
        }else{
            console.log(`🕊账号[${user.index}] 任务成功🎉`)
        }
    } catch (e) {
...
async  account(user) {
    try {
        ...
        let urlObject = {
            method: 'get',
            url: `https://consumer-api.quncrm.com/v2/member/scoreHistories?maijsVersion=1.50.0&clientId=0191a818-750c-56ee-0ba8-8442d8ff94f0&appVersion=1.94.0.2368f2fbb1c&appName=群脉电商&envVersion=release&page=1&perPage=20&listCondition.orderBy[]=-createdAt&listCondition.page=1&clientTime=2024-08-31T19:52:56.809+08:00`,
            headers: this.headers,
        };
        let { data: result } = await axios.request(urlObject)
        //console.log(result);
        if (result) {
            DoubleLog(`🕊账号[${user.index}] 账户当前积分${result.items[0].balance}🎉`)
        }else {
