 ...
 * 频道：https://t.me/Mist153
 * Date: 2024-07-05
 * cron "0 9 * * *" qchyjlb.js
 * export qchyjlb= Authorization 多账号换行或者#分隔
 */
// ============================================================================================================
const $ = new Env('雀巢会员俱乐部') 
const axios = require('axios')
const env_name = 'qchyjlb' //环境变量名字
...
!(async () => {
    await getNotice();
    await getVersion("yang7758258/ohhh154@main/qchyjlb.js");
    await main();
    await SendMsg(msg);
    ...
    async  sign(user) {
        try {
            ...
            if (result?.errcode == '200') {
                //打印签到结果
                DoubleLog(`🕊账号[${user.index}] 签到成功:获得[2]雀巢币🎉`);
            }else{
                DoubleLog(`🕊账号[${user.index}] 签到失败:失效或重复签到⛔`)
            }
        } catch (e) {
           console.log(e);
        }
...
async  account(user) {
    try {
        ...
        if (result?.errcode == '200') {
            //打印签到结果
            DoubleLog(`🕊账号[${user.index}] 当前:雀巢币[${result.data}]🎉`);
            
        }else {
            DoubleLog(`🕊账号[${user.index}] 查询失败:${result.errmsg}⛔`)
        }
        } catch (e) {
        console.log(e);
        }
